...
class MonteCarloPricing(OptionPricingModel):
    ...
    def plot_simulation_results(self, num_of_movements):
        """Plots specified number of simulated price movements and returns the figure for Streamlit."""
        import matplotlib.pyplot as plt

        fig, ax = plt.subplots(figsize=(12, 8))  # create figure and axes explicitly
        ax.plot(self.simulation_results_S[:, 0:num_of_movements])
        ax.axhline(self.K, color='k', xmin=0, xmax=self.num_of_steps, label='Strike Price')
